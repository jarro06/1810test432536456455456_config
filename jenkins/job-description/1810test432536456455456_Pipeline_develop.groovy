...
def customLib = library identifier: '1810test432536456455456_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/ORGANISATION/1810test432536456455456_lib.git',
   credentialsId: 'git-token-credentials']) 
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                    jslGit.checkout( config, "JEQP", "1810test432536456455456_Pipeline_develop", "feature-1668078556833")
                }
            }    
        }
        stage('Build') {
