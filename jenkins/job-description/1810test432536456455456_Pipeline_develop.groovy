...
def job_config = [
        job: [
                name: "1810test432536456455456_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "feature-1669887118681"
        ]
]
...
def customLib = library identifier: '1810test432536456455456_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/jarro06/1810test432536456455456_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.1810test432536456455456.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "jarro06", "1810test432536456455456", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("1810test432536456455456_Pipeline_develop");

job.with {

    authenticationToken('P0c2yx0KFHr3owx587nn7v5PgpB4kkxF')
